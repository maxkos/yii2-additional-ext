...
class RedisConnection extends Redis implements Configurable
{
     ...
     * @var int
     */
    public $serializer = self::SERIALIZER_NONE;
    ...
    public function open()
    {
        if ($this->unixSocket !== null)
            $isConnected = parent::open($this->unixSocket);
        else
            $isConnected = parent::open($this->hostname, $this->port, $this->connectionTimeout);
        ...
        $this->setOption(self::OPT_SERIALIZER, $this->serializer);
        $this->setOption(self::OPT_PREFIX, $this->prefix);
        $this->setOption(self::OPT_READ_TIMEOUT, $this->readTimeout);
        $this->setOption(self::OPT_SCAN, $this->scan);
    }
